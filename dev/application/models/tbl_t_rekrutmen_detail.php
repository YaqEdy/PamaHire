...
class tbl_t_rekrutmen_detail extends CI_Model {
  var $table = 'tbl_t_rekrutmen_detail';
  ...
  public function get_by_selected($pid_rekrutmen)
	{
		$this->db->from($this->table);
		$this->db->where('pid_rekrutmen',$pid_rekrutmen);
		$query = $this->db->get();
  ...
  public function delete_by_pid_rek($id)
	{
		$this->db->where('pid_rekrutmen', $id);
		$this->db->delete($this->table);
	}
}
